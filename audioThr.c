...
#define NUMSAMPLES       	320 //(1200)   //(1200) /*old 1024*/
...
int AUDIO_audioTskRun(void)
{
	...
	//memset(gAUDIO_ctrl.inputBuffer, 0, INPUTBUFSIZE);
...
#else
				//rc = DRV_audioWrite(&gAUDIOPLAY_ctrl.audioHndl, sharedAudioData, 160);
				rc = DRV_audioWrite(&gAUDIOPLAY_ctrl.audioHndl, sharedAudioData, rbuf.pktSize/2);
#endif
			}
